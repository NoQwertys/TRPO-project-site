...
function rename_cart_button_text() {
    return __( 'Заказать', 'woocommerce' );
}
add_filter( 'woocommerce_product_single_add_to_cart_text', 'rename_cart_button_text' );
add_filter( 'woocommerce_product_add_to_cart_text', 'rename_cart_button_text' );
add_filter( 'woocommerce_cart_button_text', 'rename_cart_button_text' );
add_filter( 'woocommerce_order_button_text', 'rename_cart_button_text' );

add_filter( 'gettext', function( $translated_text ) {
    if ( 'Просмотр корзины' === $translated_text ) {
        $translated_text = 'Просмотр заказа';
    }
	
	if ( 'Оформление заказа' === $translated_text ) {
        $translated_text = 'Оформить заказ';
    }
	
	if ( 'Корзина' === $translated_text ) {
        $translated_text = 'Заказ';
    }
	
	if ( 'Корзина пуста.' === $translated_text ) {
        $translated_text = 'Пусто';
    }
